...
$qry_rak="SELECT id_sup, nm_sup FROM sup_data";
$rak=mysql_query($qry_rak) or die (mysqli_error());
$x_ = "";
$y_ = "";
while($data=mysql_fetch_object($rak)){
	$id=$data->id_sup;
		$a = mysql_query(" SELECT COUNT(`id_brg`) AS jml_rak FROM  `br_data` WHERE  `id_sup`='$id'");
...
$graph->title->Set("Data Barang per Supplier Barang");
$graph->SetBox(true);
