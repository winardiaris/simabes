...
class barang{
	...
	function simpan_barang($id_brg,$kode_brg,$nm_brg,$id_kt_brg,$id_kualitas,$hrg_beli,$hrg_jual,$id_satuan,$stok,$stok_min,$id_rak,$id_sup,$tgl_masuk,$ket_brg,$photo_brg,$wkt_ubah){
		...
		VALUES ('$id_brg','$kode_brg','$nm_brg','$id_kt_brg','$id_kualitas','$hrg_beli','$hrg_jual','$id_satuan','$stok','$stok_min','$id_rak','$id_sup','$tgl_masuk','$ket_brg','$photo_brg','$wkt_ubah')") ;
	}
	function cek_ada($id_brg,$kode_brg){
		$qry = mysql_query("select id_brg,kode_brg FROM br_data WHERE id_brg='$id_brg' OR kode_brg='$kode_brg'") ;
		while($row = mysql_fetch_array($qry))
			$data[] = $row;
			return $data;
	}
	function hapus_barang($id_brg){
		mysql_query("DELETE FROM br_data WHERE id_brg='$id_brg' LIMIT 1") ;
	}
	function perbaharui_photo($id_brg,$photo_brg){
		mysql_query("UPDATE br_data SET photo_brg='$photo_brg' WHERE id_brg='$id_brg'") ;
	}
	...
	function tampil_br_kendaraan($id_brg){		
		$qry = mysql_query("SELECT * FROM br_data_perkendaraan WHERE id_brg='$id_brg'") ;
		while($row = mysql_fetch_array($qry))
			$data[] = $row;
			return $data;
	}
	function simpan_brg_kendaraan($id_brg,$id_kendaraan,$wkt_ubah){
		mysql_query("INSERT INTO br_data_perkendaraan (id_brg,id_kendaraan,wkt_ubah) values ('$id_brg','$id_kendaraan','$sekarang')") ;
	}
	function hapus_brg_kendaraan($id_brg){
		mysql_query("DELETE  FROM `br_data_perkendaraan` WHERE id_brg='$id_brg'") ;
	}
	
	//kategori
	function tampil_kategori(){
		$qry = mysql_query("SELECT * FROM br_kategori") ;
		while($row = mysql_fetch_array($qry))
			$data[] = $row;
			return $data;
	}
	...
	function simpan_kategori($id_kt_brg,$nm_kt_brg,$wkt_ubah){
		mysql_query("INSERT INTO br_kategori values ('$id_kt_brg','$nm_kt_brg','$wkt_ubah') ") ;		
	}
	function cek_kategori($id_kt_brg){
		$qry = mysql_query("SELECT * FROM br_kategori WHERE id_kt_brg='$id_kt_brg'") ;
		while($row = mysql_fetch_array($qry))
			$data[] = $row;
			return $data;
	}
	function hapus_kategori($id_kt_brg){
		mysql_query("DELETE FROM br_kategori WHERE id_kt_brg='$id_kt_brg'") ;
	}
	function perbaharui_kategori($id_kt_brg,$nm_kt_brg,$wkt_ubah){
		mysql_query("UPDATE br_kategori SET nm_kt_brg='$nm_kt_brg',wkt_ubah='$wkt_ubah' WHERE id_kt_brg='$id_kt_brg' ") ;
	}
	
	//jenis kendaraan
	function tampil_jenis_kendaraan(){
		$qry = mysql_query("SELECT * FROM br_kendaraan") ;
		while($row = mysql_fetch_array($qry))
			$data[] = $row;
			return $data;
	}
	...
	function simpan_jenis_kendaraan($id_kendaraan,$kendaraan,$wkt_ubah){
		mysql_query("INSERT INTO br_kendaraan VALUES('$id_kendaraan','$kendaraan','$wkt_ubah')") ;
		
	}
	function cek_jenis_kendaraan($id_kendaraan){
		$qry = mysql_query("SELECT * FROM br_kendaraan WHERE id_kendaraan='$id_kendaraan'") ;
		while($row = mysql_fetch_array($qry))
			$data[] = $row;
			return $data;
	}
	function hapus_jenis_kendaraan($id_kendaraan){
		mysql_query("DELETE FROM br_kendaraan WHERE id_kendaraan='$id_kendaraan' ") ;
	}
	function perbaharui_jenis_kendaraan($id_kendaraan,$kendaraan,$wkt_ubah){
		mysql_query("UPDATE br_kendaraan SET kendaraan='$kendaraan', wkt_ubah='$wkt_ubah' WHERE id_kendaraan='$id_kendaraan'") ;
	}
	
	//kualitas
	function tampil_kualitas(){
		$qry = mysql_query("SELECT * FROM br_kualitas") ;
		while($row = mysql_fetch_array($qry))
			$data[] = $row;
			return $data;
	}
	...
	function simpan_kualitas($id_kualitas,$kualitas,$wkt_ubah){
		mysql_query("INSERT INTO br_kualitas VALUES('$id_kualitas','$kualitas','$wkt_ubah')") ;		
	}
	function cek_kualitas($id_kualitas){
		$qry = mysql_query("SELECT * FROM br_kualitas WHERE id_kualitas='$id_kualitas'") ;
		while($row = mysql_fetch_array($qry))
			$data[] = $row;
			return $data;
	}
	function hapus_kualitas($id_kualitas){
		mysql_query("DELETE FROM br_kualitas WHERE id_kualitas='$id_kualitas'") ;
	}
	function perbaharui_kualitas($id_kualitas,$kualitas,$wkt_ubah){
		mysql_query("UPDATE br_kualitas SET kualitas='$kualitas',wkt_ubah='$wkt_ubah' WHERE id_kualitas='$id_kualitas' ") ;
	}
	
	//satuan
	function tampil_satuan(){
		$qry = mysql_query("SELECT * FROM br_satuan") ;
		while($row = mysql_fetch_array($qry))
			$data[] = $row;
			return $data;
	}
	...
	function simpan_satuan($id_satuan,$satuan,$wkt_ubah){
		mysql_query("INSERT INTO br_satuan VALUES('$id_satuan','$satuan','$wkt_ubah')") ;
	}
	function cek_satuan($id_satuan){
		$qry = mysql_query("SELECT id_satuan FROM br_satuan WHERE id_satuan='$id_satuan'") ;
		while($row = mysql_fetch_array($qry))
			$data[] = $row;
			return $data;
	}
	function hapus_satuan($id_satuan){
		mysql_query("DELETE FROM br_satuan WHERE id_satuan='$id_satuan'") ;
	}
	function perbaharui_satuan($id_satuan,$satuan,$wkt_ubah){
		mysql_query("UPDATE br_satuan SET satuan='$satuan', wkt_ubah='$wkt_ubah' WHERE id_satuan='$id_satuan' ") ;
	}
	
	//rak
	function tampil_rak(){
		$qry = mysql_query("SELECT * FROM br_rak") ;
		while($row = mysql_fetch_array($qry))
			$data[] = $row;
			return $data;
	}
	...
	function simpan_rak($nm_rak,$ket,$wkt_ubah){
		mysql_query("INSERT INTO br_rak VALUES ('','$nm_rak','$ket','$wkt_ubah')") ;
	}
	function cek_rak($nm_rak){
		$qry = mysql_query("SELECT * FROM br_rak WHERE nm_rak='$nm_rak'") ;
		while($row = mysql_fetch_array($qry))
			$data[] = $row;
			return $data;
	}
	function hapus_rak($id_rak){
		mysql_query("DELETE FROM br_rak WHERE id_rak='$id_rak'") ;
	}
	function perbaharui_rak($id_rak,$nm_rak,$ket,$wkt_ubah){
		mysql_query("UPDATE br_rak SET nm_rak='$nm_rak',ket='$ket',wkt_ubah='$wkt_ubah' WHERE id_rak='$id_rak' ") ;
	}
	...
	function simpan_penyalur($nm_sup,$almt_sup,$telp_sup,$wkt_ubah){
		mysql_query("INSERT INTO sup_data VALUES ('','$nm_sup','$almt_sup','$telp_sup','$wkt_ubah')") ;
	}
	function cek_penyalur($nm_sup){
		$qry = mysql_query("SELECT * FROM sup_data WHERE nm_sup='$nm_sup'") ;
		while($row = mysql_fetch_array($qry))
			$data[] = $row;
			return $data;
	}
	function hapus_penyalur($id_sup){
		mysql_query("DELETE FROM sup_data WHERE id_sup='$id_sup'") ;
	}
	function perbaharui_penyalur($id_sup,$nm_sup,$almt_sup,$telp_sup,$wkt_ubah){
		mysql_query("UPDATE sup_data SET nm_sup='$nm_sup',almt_sup='$almt_sup',telp_sup='$telp_sup',wkt_ubah='$wkt_ubah' WHERE id_sup='$id_sup' ") ;
	}
	
	//stok kurang
	function tampil_stok_kurang(){
		$qry = mysql_query("SELECT * FROM br_data  WHERE stok<=stok_min  AND dipesan='0' ORDER BY id_sup ASC") ;
		while($row = mysql_fetch_array($qry))
			$data[] = $row;
			return $data;
	}
	...
	function simpan_pembelian_detail($no_pes,$tgl_pes,$id_sup,$id_brg,$hrg_brg,$jml_brg,$total,$wkt_ubah){
		mysql_query("INSERT INTO br_pembelian_detail VALUES('','$no_pes','$tgl_pes','$id_sup','$id_brg','$hrg_brg','$jml_brg','$total','0','$wkt_ubah')") ;
	}
	function perbaharui_pembelian_detail($no_pes,$id_brg){
		mysql_query("UPDATE br_pembelian_detail SET diterima='1' WHERE no_pes='$no_pes' AND id_brg='$id_brg'") ;
	}
	...
	function simpan_pembelian($no_pes,$tgl_pes,$id_sup,$total_pembayaran,$diterima,$id_pengguna,$wkt_ubah){
		mysql_query("INSERT INTO br_pembelian VALUES('$no_pes','$tgl_pes','$id_sup','$total_pembayaran','$diterima','$id_pengguna','$wkt_ubah')") ;
	}
	function perbaharui_pembelian($no_pes,$total_pembayaran){
		mysql_query("UPDATE br_pembelian SET total_pembayaran='$total_pembayaran', diterima='1' WHERE no_pes='$no_pes' ") ;
	}
	
	
	//keuangan
	function simpan_keuangan($tgl,$ket,$masuk,$keluar){
		mysql_query("INSERT INTO keuangan VALUES('','$tgl','$ket','$masuk','$keluar')") ;
	}
	...
	function tambah_sementara($id_sementara,$value){
		mysql_query("INSERT INTO sementara VALUES ('$id_sementara','$value')") ;
	}
	function hapus_sementara($id_sementara,$value){
		mysql_query("DELETE FROM sementara WHERE id_sementara='$id_sementara' AND value='$value'") ;
	}
